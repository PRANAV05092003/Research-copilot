...
def mock_compressor():
    ...
    compressor.compress.return_value = [{"chunk_id": "1", "text": "Compressed text 1", "score": 0.9}]
    return compressor
...
@pytest.mark.asyncio
async def test_conversation_service_create(mock_conv_repo, mock_rag_service, mock_llm, mock_verifier):
...
@pytest.mark.asyncio
async def test_conversation_service_send_message(mock_conv_repo, mock_rag_service, mock_llm, mock_verifier):
    service = ConcreteConversationService(mock_conv_repo, mock_rag_service, mock_llm, mock_verifier)
    conv_id = uuid.uuid4()
