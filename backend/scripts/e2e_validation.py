...
async def run_e2e():
    ...
    async with httpx.AsyncClient() as client:
        # Check Health
        health = await measure(
            "API Startup / Health", client.get("http://localhost:8000/api/v1/health/live")
        )
        ...
        res = await client.post(
            f"{API_BASE}/auth/refresh", cookies={"refresh_token": refresh_cookie}
        )
        assert res.status_code == 200
        data = res.json()
        ...
        print("6. Logout")
        res = await client.post(
            f"{API_BASE}/auth/logout", cookies={"refresh_token": new_refresh_cookie}
        )
        assert res.status_code == 200
        ...
        if process.returncode == 0:
            try:
                ...
                assert db_res["embedding_dim"] == 384, (
                    f"Invalid embedding dimension: {db_res['embedding_dim']}"
                )
                print(
                    "DB Verification: Paper, Chunks, and Embeddings (dim 384) verified successfully!"
                )
        ...
        res = await measure(
            "LangGraph First Response Latency",
            client.post(f"{API_BASE}/conversations/{conv_id}/messages", json=msg),
        )
        ...
        if msg_out["citations"]:
            ...
            assert citation["verification_status"] in ["verified", "unverified", "failed"], (
                f"Invalid status: {citation['verification_status']}"
            )
        ...
        res = await measure(
            "Complete Research-Job Latency",
            client.post(f"{API_BASE}/research/review", json={"topic": "Hello"}),
        )
