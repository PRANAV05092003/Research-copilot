...
class PaperIngestionService(abc.ABC):
    @abc.abstractmethod
    async def enqueue_paper(self, file_bytes: bytes, filename: str, user_id: uuid.UUID, workspace_id: uuid.UUID) -> uuid.UUID:
        pass

class RAGService(abc.ABC):
    @abc.abstractmethod
    async def hybrid_search(self, query: str, workspace_id: uuid.UUID, top_k: int = 10) -> list[dict[str, Any]]:
        pass

class ResearchAgentService(abc.ABC):
    @abc.abstractmethod
    async def generate_deep_review(self, topic: str, user_id: uuid.UUID, workspace_id: uuid.UUID) -> uuid.UUID:
        pass

class ConversationService(abc.ABC):
    @abc.abstractmethod
    async def create_conversation(self, title: str, user_id: uuid.UUID, workspace_id: uuid.UUID, mode: str) -> Conversation:
        pass
...
class ConcreteRAGService(RAGService):
    def __init__(self, paper_repo: PaperRepository, retriever: BaseRetriever, compressor: ContextCompressor):
        self.paper_repo = paper_repo
    ...
    async def hybrid_search(self, query: str, workspace_id: uuid.UUID, top_k: int = 10) -> list[dict[str, Any]]:
        ...
        raw_chunks = await self.retriever.retrieve(query=query, top_k=top_k * 2, workspace_id=workspace_id)
