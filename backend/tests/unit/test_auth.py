...
def test_login_valid_credentials():
    ...
    response = client.post(
        "/api/v1/auth/login", json={"email": "test@example.com", "password": "ValidPass123!"}
    )
...
def test_logout():
    ...
    assert db_token.revoked_at is not None
    assert (
        response.cookies.get("refresh_token") is None or response.cookies.get("refresh_token") == ""
    )
...
def test_get_me_valid():
    user = User(
        id=uuid.uuid4(), email="test@example.com", is_active=True, created_at=datetime.now(UTC)
    )
...
def test_get_me_inactive():
    user = User(
        id=uuid.uuid4(), email="test@example.com", is_active=False, created_at=datetime.now(UTC)
    )
